    ...
    if request.method == "PATCH":
        ...
        try:
            if jwt.decode(
                ...
                result = cursor.fetchone()
                cursor.execute(
                 "SELECT emp_id FROM employee_details WHERE emp_id = %s AND role = 'manager'",
                (new_data["userid"],),
                )
    ...
    if request.method == "PATCH":
        ...
        try:
            if jwt.decode(token.split(" ")[1], app.config['SECRET_KEY'], algorithms=['HS256']):
                ...
                cursor.execute("UPDATE employee_details SET emp_id=%s WHERE emp_name=%s", (new_data['id'], new_data['username']))
                get_db().commit()
                return jsonify(message="updated successfully")
        except jwt.ExpiredSignatureError:
...
def search():
    ...
    if request.method == "PATCH":
        ...
        user_details2 = []
        if search=='name':
                cursor.execute("SELECT emp_id,emp_name,emp_email,role FROM employee_details WHERE emp_id=%s", (new_data['userid'],))
                users=cursor.fetchall()
                for user in users:
                    user_details.append(
                    { "userid": user[0], "employee_name": user[1],"employee_email":user[2],"role":user[3]})
                return jsonify(user_details), 401
        elif search=='employee':
                    cursor.execute("SELECT emp_id,emp_name,emp_email,role FROM employee_details")
                    users=cursor.fetchall()
                    for user in users:
                       user_details2.append(
                       { "userid": user[0], "employee_name": user[1],"employee_emal":user[2],"role":user[3]})
        ...
        try:
            if jwt.decode(token.split(" ")[1], app.config['SECRET_KEY'], algorithms=['HS256']):
                ...
                user_details1 = []
                if search=='manager':
                    cursor.execute("SELECT emp_id, emp_name, emp_email FROM employee_details WHERE reporting_id = %s",(new_data["userid"]),)
                    users=cursor.fetchall()
                    for user in users:
                ...
                # return jsonify({message="only manager can view this data"}), 401
                    ...
                    cursor.execute("SELECT emp_id, emp_name, emp_email,reporting_id FROM employee_details WHERE emp_id = %s",(new_data["userid"]),)
                    users=cursor.fetchall()
                    for user in users:
                        user_details1.append(
                        { "username": user[0], "email": user[1],"employee_email": user[2],"manager_id":user[3]})
                    print("hi")
                    return jsonify(user_details1)
